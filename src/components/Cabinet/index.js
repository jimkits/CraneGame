...
export function buildCabinet(scene, world) {
  ...
  // All lights tucked at ceiling level — hidden behind LED strip geometry, no visible dots
  const iLight = new THREE.PointLight(0xeef5ff, 10, 9);
  iLight.position.set(0, H - 0.06, 0); scene.add(iLight);

  const fillL = new THREE.PointLight(0xeef5ff, 5, 7);
  fillL.position.set(-W / 3, H - 0.06, 0); scene.add(fillL);
  const fillR = new THREE.PointLight(0xeef5ff, 5, 7);
  fillR.position.set( W / 3, H - 0.06, 0); scene.add(fillR);

  const floorLight = new THREE.PointLight(0x4466ff, 5, 5);
  floorLight.position.set(0, H - 0.06, D / 3); scene.add(floorLight);
